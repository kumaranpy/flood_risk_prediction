...
Architectural Guarantees:
  ...
  4. Performs deterministic 80/20 stratified train/test split.
"""
...
from src.config import get_paths_config, get_training_config, get_paths
...
def run_data_preparation() -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    raw_data_path = paths["raw_data"] / "flood_factors.csv"
    ...
    if X_raw.isnull().any().any() or np.isinf(X_raw.select_dtypes(include=[np.number])).any().any():
        logger.warning("Missing or infinite values found in raw data. Will impute per-fold after split.")
    ...
    # Compute imputation statistics STRICTLY ON TRAINING SET and apply to both
    ...
    medians_path = get_paths()["models"] / "feature_medians.json"
    save_json(train_medians.to_dict(), medians_path)
    logger.info(f"Saved feature medians to: {medians_path}")

    # Step 2: Compute tertile boundaries STRICTLY ON TRAINING SET
    target_bins = compute_target_bins(y_train_cont)
    logger.info(f"Computed training empirical target bins: {target_bins}")

    target_bins_path = get_paths()["models"] / "target_bins.json"
    with open(target_bins_path, "w", encoding="utf-8") as f:
        json.dump(target_bins, f, indent=2)
    logger.info(f"Saved target bins metadata to: {target_bins_path}")
    ...
    logger.info(y_train_discrete.value_counts(normalize=True).to_string())

    logger.info("\nTest class balance:")
    logger.info(y_test_discrete.value_counts(normalize=True).to_string())
    ...
    train_df.to_csv(get_paths()["splits"] / "train.csv", index=False)
    test_df.to_csv(get_paths()["splits"] / "test.csv", index=False)

    logger.info(f"Persisted raw training partition to: {get_paths()['splits'] / 'train.csv'}")
    logger.info(f"Persisted raw test partition to:     {get_paths()['splits'] / 'test.csv'}")
    logger.info("PHASE 1 COMPLETED SUCCESSFULLY (No Global Scaling or Pre-Split Leakage)")
