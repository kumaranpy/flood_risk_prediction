...
def create_kernel_explainer(
    ...
    Uses a lambda wrapper to convert numpy arrays back to named DataFrames
    before calling predict_proba — this avoids the
    "property 'feature_names_in_' of 'Pipeline' object has no setter" error
    that occurs when SHAP's KernelExplainer tries to set Pipeline attributes.
    """
    ...
    # This prevents SHAP from hitting the read-only feature_names_in_ property
    def predict_fn(x: np.ndarray) -> np.ndarray:
        df = pd.DataFrame(x, columns=feature_cols)
        return pipeline.predict_proba(df)

    # Use kmeans summary for background to speed up kernel SHAP
    try:
        background_summary = shap.kmeans(background_sample.values, min(10, len(background_sample)))
    except Exception:
        background_summary = background_sample.values

    # Create explainer using the wrapped predict function
    ...
    if n_classes == 3 and isinstance(shap_values, list):
        ...
        for i, (cls_name, cls_shap) in enumerate(zip(class_names, shap_values)):
            ...
            if save_path:
                ...
                logger.info(f"  Saved SHAP summary to: {cls_path}")
            plt.close()
    else:
    ...
    try:
        ...
        if isinstance(shap_values, list):
            if sample_idx >= len(shap_values[class_idx]):
                logger.warning(f"  Skipping waterfall: sample_idx {sample_idx} >= {len(shap_values[class_idx])}")
                return
...
def get_top_features_per_prediction(
    ...
    for i in range(n_samples):
        ...
        if isinstance(shap_values, list):
            ...
            # Could be Explanation object or numpy array
            if hasattr(shap_values, 'values'):
                vals = shap_values.values
                if vals.ndim == 3:
                    class_idx = ["Low", "Medium", "High"].index(pred_class)
                    sample_shap = vals[i, :, class_idx]
                else:
                    sample_shap = vals[i]
            else:
                sample_shap = shap_values[i] if shap_values.ndim > 1 else shap_values
...
def run_explainability_demo():
    ...
    setup_logging()
    logger.info("=" * 60)
    logger.info("SHAP EXPLAINABILITY DEMONSTRATION (KernelExplainer)")
    logger.info("=" * 60)
    ...
    # Load test data
    test_path = get_paths()["splits"] / "test.csv"
    ...
    plot_shap_summary(
        ...
        save_path=get_paths()["plots"] / "shap_summary.png"
    )
    ...
    if high_risk_indices:
        plot_shap_waterfall(
            ...
            save_path=get_paths()["plots"] / "shap_waterfall_high.png"
        )
    ...
    for result in top_features[:10]:
        logger.info(f"  Sample {result['sample_idx']} ({result['predicted_class']}):")
        if result['top_features']:
            for feat in result['top_features']:
                logger.info(f"    {feat['feature']}: SHAP={feat['shap_value']:+.4f} (value={feat['feature_value']:.2f})")
        else:
    ...
    if isinstance(shap_values, list):
        for i in range(min(5, len(X_sample))):
            ...
            logger.info(f"  Sample {i} (Predicted: {pred_class}):")
            for idx in top_indices:
                logger.info(f"    {feature_names[idx]}: SHAP={sample_shap[idx]:+.4f}")
            logger.info("")
